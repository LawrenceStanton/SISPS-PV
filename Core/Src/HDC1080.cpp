...
/**
 * @defgroup HDC1080 Software Reset Configuration Write Value.
 * 
 * @{ 
 */
...
/**
 * @defgroup HDC1080 Measurement Conversion Times (ms)
 * 
...
uint64_t HDC1080::getSerialID(){
	uint16_t serialID[3];
	for(unsigned int i = 0; i < 3; i++) serialID[i] = getRegister(HDC1080_SERIAL_ID_ADDR + i);
 ...
 * for STM32-based systems using the ST Microelectronics STM32 Hardware Abstraction Layer (HAL) for a single HDC1080.
 *************************************************************************************************************************/
