...
/**
 * @defgroup HDC1080 I2C Constants
 * 
 * @{ 
 */
...
/**
 * @defgroup HDC1080 Register Map and Related Constants
 * 
 * @{ 
 */
	/* Register Addresses */
...
/**
 * @defgroup HDC1080 Floating Variables Implementation Enable
 * 
 * @note Define this constant in order to implement variables and methods for computing the floating point measurement values.
 * @{ 
 */
...
/**
 * @defgroup HDC1080 Byte Reversal Casting
 * 
...
/**
 * @brief Main class representing the HDC1080.
 * 
 * @note Refer to mehtod source code for detailed mehtod information.
 * 
 */
class HDC1080{
	...
	*/
	...
	void setConfig(AcqModeConfig acqMode, TempResolutionConfig tRes, HumidityResolutionConfig hRes, HeaterConfig heater);
	 ...
	 * 
	 * @note This mehtod is primarily used when reading general information but not measurements (due to measurement conversion time).
	 * 
	 ...
	 * @note This method must be implemented by host application and based on the specific host system infrastructure.
	 * 
	 * @note This methods is primarily used when fetching measurements, which require separation between memory addressing
	 * 		 and data transaction in order to insert a measurement conversion time delay.
	 * 
	 * @note The HDC1080 will auto-increment the address pointer when writing multiple registers in one transaction.
	 * 		 There is no need to manually increment the memory address pointer.
	 * @note ID should be used to determine which physical I2C interface to use if there are multiple HDC1080s.
 ...
 */
enum class HDC1080::HeaterConfig : uint16_t {
	OFF = 0x2000u,
	ON  = 0x0000u
};
 ...
 */
enum class HDC1080::AcqModeConfig : uint16_t{
	SINGLE = 0x0000u,
	DUAL   = 0x0100u
};
 ...
 */
enum class HDC1080::TempResolutionConfig : uint16_t{
	A_14BIT = 0x0000u,
	A_11BIT = 0x0040u
};
 ...
 */
enum class HDC1080::HumidityResolutionConfig : uint16_t{
	A_14BIT = 0x0000u,
	A_11BIT = 0x0010u,
	A_8BIT  = 0x0020u
};
